...
$Existencia=15;
$sql = "INSERT INTO $Seccion (Nombre,Descripcion,Precio, Existencia, Vendedor)
VALUES ('$Nombre', '$Descripcion', '$Precio', '$Existencia', '$Vendedor')";
...
$consultados =$conexion->query("SELECT * from $Seccion WHERE Nombre='$Nombre'");       
if($resultado = mysqli_fetch_array($consultados)){
    $var=$resultado['id'];
    $imagen=$var.'.jpg';
...
if ( isset( $_FILES ) && isset( $_FILES['myfile'] ) && !empty( $_FILES['myfile']['name'] && !empty($_FILES['myfile']['tmp_name']) ) ) {
	...
	$destination = __DIR__.'/images/'.$Seccion.'/'.$imagen;
	...
	if ( ! @move_uploaded_file($source, $destination ) ) {
      ...
      <div class='android-content mdl-layout__content'>
        ...
        <div class='android-be-together-section1 mdl-typography--text-center'>
          ...
          <div class='android-font android-sub-slogan'>Tu producto ha sido subido satisfactoriamente
</div>";
